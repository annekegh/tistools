...
def construct_tau_vector(N, NS, taumm, taump, taupm, taupp):
    """
    Construct a tau vector from given path lengths.
    ...
    taumm, taump, taupm, taupp : list of float
        Path lengths for different segments.

    Returns
    -------
    np.ndarray
        Constructed tau vector.
...
def set_tau_first_hit_M_distrib(pe, do_last=True):
    """
    Set the average path length before the middle interface is crossed for each path type.
    ...
    do_last : bool, optional
        If True, also compute tau2. Default is True.
...
def get_tau1_path(orders, ptype, intfs):
    ...
    -------
    int
        Number of steps to cross the M interface.
...
def get_tau2_path(orders, ptype, intfs):
    """
    Return the number of steps in the path after the last crossing of the M interface.
    ...
    -------
    int
        Number of steps after the last crossing of the M interface.
...
def collect_tau1(pathensembles):
    """
    Compute and collect average time to hit the M interface for all path ensembles.
    ...
    -------
    tuple
        Average times to hit the M interface (tau1_mm, tau1_mp, tau1_pm, tau1_pp).
    """
    print("Collect tau1")
...
def collect_tau2(pathensembles):
    """
    Compute and collect average time after hitting the M interface for all path ensembles.
    ...
    -------
    tuple
        Average times after hitting the M interface (tau2_mm, tau2_mp, tau2_pm, tau2_pp).
    """
    print("Collect tau2")
...
def collect_taum(pathensembles):
    """
    Compute and collect average time between the first and last hit of the M interface for all path ensembles.
    ...
    -------
    tuple
        Average times between the first and last hit of the M interface (taum_mm, taum_mp, taum_pm, taum_pp).
    """
    print("Collect taum")
...
def set_taus(pe):
    """
    Set the average path lengths before and after the middle interface is crossed, and the average total path length.
