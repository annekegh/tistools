    ...
    for i in range(len(interfaces)):
        for k in range(len(interfaces)):
            ...
            if i == k:
                ...
                counts += [np.sum(w_path[k]["ends"][i][k:]), np.sum(w_path[k]["ends"][i][k-1:])]
                print(i,k,np.sum(w_path[k]["ends"][i][k:])/np.sum(w_path[k]["ends"][i][k-1:]), np.sum(w_path[k]["ends"][i][k-1:]))
            elif i > k:
                if k+2 > len(interfaces)-1:
                    break
                counts += [np.sum(w_path[k+2]["ends"][i][:k+1]), np.sum(w_path[k+2]["ends"][i][:k+2])]
                print (i,k,np.sum(w_path[k+2]["ends"][i][:k+1])/np.sum(w_path[k+2]["ends"][i][:k+2]), np.sum(w_path[k+2]["ends"][i][:k+2]))
        ...
        for k in range(len(interfaces)):
            ...
            if i < k:
                for pe_i in range(i+1,k+2):
                    if pe_i > len(interfaces)-1:
                        break
                    counts += [w_path[pe_i]["ends"][i][k], np.sum(w_path[pe_i]["ends"][i][i:])]
            elif k < i:
                if i == len(interfaces)-1:
                    p[i][k] = 0
                    continue
                for pe_i in range(k+1,i+2):
                    if pe_i > len(interfaces)-1:
                        break
                    counts += [w_path[pe_i]["ends"][i][k], np.sum(w_path[pe_i]["ends"][i][:i+1])]
            else:
                if i == 0:
